...
function CrewContent() {
  ...
  return (
    <div className="grid lg:grid-cols-2 lg:gap-y-[3.2rem] ">
      <div className="flex flex-col justify-between gap-y-[2.4rem] lg:gap-y-[4rem] ">
        <div
          role="contentinfo"
          className="flex flex-col justify-center h-full py-[2.4rem] gap-y-[2.4rem] text-center lg:text-start text-light-blue"
        >
          <div className="flex flex-col gap-y-[.8rem] uppercase text-default">
            <p className="text-[1.8rem] md:text-[2.4rem] lg:text-[3.2rem] opacity-50">
              {crewMember?.role}
            </p>
            <h1 className="text-[2.4rem] md:text-[4rem] lg:text-[5.6rem]">{crewMember?.name}</h1>
          </div>
          <p className="text-[1.5rem] md:text-[1.6rem] lg:text-[1.8rem]">{crewMember?.bio}</p>
        </div>
        <CrewNav />
      </div>
